...
export function RedemptionModal({
  ...
  // Polling Logic
  useEffect(() => {
    if (!isOpen || !redemption || status === "CONFIRMED" || timeLeft === 0)
      return;
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent
        side="bottom"
        className="h-[90vh] sm:h-[650px] rounded-t-3xl border-none p-6 outline-none overflow-y-auto"
      >
        <SheetHeader className="items-center text-center mb-6">
          <div
            className={`p-3 rounded-full mb-2 ${isConfirmed ? "bg-green-100" : "bg-primary/10"}`}
          >
            {isConfirmed ? (
        ...
        <div className="flex flex-col items-center justify-center gap-6">
          {!isConfirmed ? (
            <>
                ...
                <QRCodeSVG
                  value={
                    process.env.NODE_ENV === "development"
                      ? `http://localhost:3000/${redemption.token}`
                      : redemption.token
                  }
                  size={200}
                  level="H"
              ...
              <div
                className={`flex items-center gap-2 px-6 py-2 rounded-full font-mono text-lg font-bold ${timeLeft < 20 ? "bg-red-50 text-red-600 animate-pulse" : "bg-slate-100 text-slate-700"}`}
              >
          ...
          <div className="w-full max-w-sm space-y-3 pt-2">
            ...
            <div className="flex justify-between items-center text-xs border-b border-slate-100 pb-2 text-slate-400">
              ...
              <span className="font-mono">
                {redemption.redemption_id.split("-")[0].toUpperCase()}
              </span>
            </div>
          </div>
